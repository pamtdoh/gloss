...
3. A browser UI — the only place a browser appears — gives the human fast,
   keyboard-driven browsing, text comments (anchored to a selection or whole-fact, with one-tap
   "quick comment" presets like "not needed" / "simplify" / "defer") and
   anchored questions.
4. When the human finishes, the agent is notified, reads the comments
...
5. When the human is happy with the current state of the facts, they approve
   — once, at the end. The approved revision is plain files that any agent
...
- **The viewer is for humans only.** It browses, annotates, asks, and
  approves. No creation forms, no agent progress, no integration hooks.
...
**Revisions are standalone copies.** A review is a directory holding
numbered revisions. Revision `1` is generated by the agent as a tree of fact
files. Each later revision starts as a plain copy of the previous one — no
shared data, no pointers, no lineage metadata between revisions.

**Reviewing writes into the revision.** The human's comments and
questions are persisted as sidecar files next to the facts
they concern (schema in section 6), so they travel with revision copies. A
...
revision, reads the sidecars wholesale, and resolves them by rewriting,
amending, or removing fact files. When the agent judges an item resolved in
the new revision, it deletes that item from the sidecar — and the sidecar
file itself once nothing remains. A revision with no sidecars is one where
...
with revision copies until resolved. During a session the agent can answer
questions live (milestone 4): the viewer reloads answers as they land,
without the human finishing the review.

**Approval is one act, at the end: promote the revision.** When the human is
satisfied, the accepted revision is copied to `approved/` — from the viewer's
...
- Blocks until the reviewer clicks **Finish review** (or approves), then
  exits 0 and prints a JSON summary: comment count, open questions,
  approval status.
- Stdout is always JSONL events (`session.started`, `question.asked`,
  `session.finished`) followed by the final summary line. Claude Code runs
  the command under a single stdout monitor: each event line is a
  notification (live Q&A rides on `question.asked`), and the process
  exiting is the completion signal. One listener, no push infrastructure,
  no webhooks, no separate polling loop.
...
copying revisions, answering questions, promoting the approved revision —
is agents reading and writing ordinary files, guided by the skills.

## 6. File conventions and the review-state schema

The layout is flat: reviews sit directly under `.gloss/`, revisions are
...
Inside a revision, the agent chooses the tree. One condensed fact per `.md`
...
claim, Mermaid diagrams for flows, and images stored inside the revision
directory and referenced relatively (they travel with revision copies).
The viewer renders all of it safely and never fetches beyond the session.

```
    2/
      _review.json            # optional: revision-level notes, same schema
      flows/
...
```json
{
  "items": [
    ...
    { "id": "c2", "type": "comment", "text": "Reads well — keep facts this small." }
  ]
}
```

- `items` carry two types: `comment` and `question` (`annotation` is a
  legacy third type, rendered and counted as a comment). Comments anchor
  to a selection or cover the whole fact. There is no decision field:
  what were decisions are the viewer's "quick comment" presets — one-tap
  whole-fact comments with canned text ("Not needed." / "Simplify." /
  "Defer."), editable like any comment. An absent sidecar means the fact
  stands as written — silence is agreement.
...
file has nothing left to say, it is deleted. Revision-level notes not tied
to one fact go in `_review.json` at the revision root, same schema minus
anchors.
...
- **`gloss`** — the primary entry point. From conversation:
  ...
  revision `1`; run `session` and wait; on finish, read the sidecars and
  propose next steps. To iterate: copy the revision, resolve sidecar items
  by editing facts, delete what's resolved, run another session. On
  approval, copy the accepted revision to `approved/`.
...
- A table view: a directory's direct children as rows, so a run of small
  facts can be reviewed (and bulk quick-commented) in one screen.
- Keyboard-driven throughout: J/K navigation and quick-comment keys.
- Text-selection comments and anchored questions (writing the sidecar
  schema above); comments and questions can be edited and deleted, not
  just undone.
- A compact review panel that doesn't crowd the fact being read.
- Safe rendering of rich Markdown (GFM tables, fenced code, in-revision
  images, Mermaid diagrams), reduced-motion and accessibility-clean,
  revision switching, live thread reload, a **Finish review** action that
  ends the session and wakes the agent, and an approve action that
  promotes the revision to `approved/`.
...
- Facts that differ from the previous revision carry a "changed" badge,
...
- A three-way tree scope — All | Changed | Raised — in a pinned header
  ...
  revision, including facts the agent deleted (read-only "ghost" rows
  resurrected from the previous revision's copy); Raised shows facts
...
1. **M1 — Files + review generation.** `gloss init`, the conventions in
   ...
   revision of facts an agent and human can both read. Exit: a review can be
...
3. **M3 — Iteration + approval + implement.** Revision copying, sidecar
...
- **Name**: `gloss` — folder, CLI binary, and `.gloss/` directory. Renamed
  ...
  a text, which is what a review item is. Historical revisions under
...
- **Flat layout**: `.gloss/<review>/<n>/`; revisions are standalone
  copies; silence is agreement.
- **Approval by promotion**: copying the accepted revision to `approved/`
...
- **No decision field** (owner round-3 self-review, 2026-08-09): sidecars
  ...
  decisions survive solely as the viewer's quick-annotate presets that
  create whole-fact annotations. Supersedes the earlier decision-set
  entries.
- **Comment is the word** (round 4, reversing round 3's pick): items are
...
- **Rich facts** (same): tables, code, diagrams, and in-revision images are
...
- **Seen state is viewer-local** (same): progress tracking lives in the
  ...
  revision iteration and clears exactly where a fact changed.
- **Three-way tree scope with deletion ghosts** (owner picked in-thread,
  ...
  direction); no persistence across revision switches.
