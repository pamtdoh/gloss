...
3. A browser UI — the only place a browser appears — gives the human fast,
   keyboard-driven browsing, text annotations (with one-tap "quick
   annotate" presets like "not needed" / "simplify" / "defer") and
   anchored questions.
...
**Reviewing writes into the snapshot.** The human's annotations and
...
- Blocks until the reviewer clicks **Finish review** (or approves), then
  exits 0 and prints a JSON summary: annotation count, open questions,
  approval status.
...
```json
{
  "items": [
    ...
    { "id": "a2", "type": "annotation", "text": "Reads well — keep facts this small." }
  ]
}
```

- `items` carry two types: `annotation` and `question` (`comment` is a
  legacy third type, rendered and counted as an annotation). There is no
  decision field (owner, round 3): what were decisions are the viewer's
  "quick annotate" presets — one-tap whole-fact annotations with canned
  text ("Not needed." / "Simplify." / "Defer."), editable like any
  annotation. An absent sidecar means the fact stands as written —
...
- A table view: a directory's direct children as rows, so a run of small
  facts can be reviewed (and bulk quick-annotated) in one screen.
- Keyboard-driven throughout: J/K navigation and quick-annotate keys.
...
- **No decision field** (owner round-3 self-review, 2026-08-09): sidecars
  ...
  entries.
- **Comment merged into annotation** (same round): a comment was an
  unanchored annotation by another name; items are `annotation` and
  `question`. Legacy `comment` items render as annotations.
