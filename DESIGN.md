...
**Reviewing writes into the snapshot.** The human's annotations, comments,
and questions are persisted as sidecar files next to the facts
...
- Blocks until the reviewer clicks **Finish review** (or approves), then
  exits 0 and prints a JSON summary: annotation and comment counts, open
  questions, approval status.
